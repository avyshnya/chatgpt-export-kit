...
#
# Запуск ІЗ ТЕКИ ПРОСТОРУ (вона і є корінь):  TARGET="no-project" bash <шлях>/ingest.sh
# Змінні середовища (шляхи — через змінні, без хардкоду):
#   ARC     корінь = тека простору   (за замовч. поточна тека, $PWD = <парасолька>/<простір>/)
#   DL      тека завантажень Chrome  (за замовч. $HOME/Downloads)
#   PREFIX  префікс ndjson-файлів    (за замовч. chats; той самий, що __kitPrefix)
#   TARGET  тека призначення відносно ARC (ОБОВ'ЯЗКОВА, напр. "no-project")
